...
def process_p_model(line):
    ...
    if event_direction == '<':
        ...
        # model_p_execve.add([re.findall(r'ptid=(\S+)', line)[0].split('(')[1].rstrip(')'), parts[3], attr_token_bag])
...
def perf_info_calculation():
    ...
    oper = os.path.getsize('oper_proc_client.csv') / 1024
    with open("perf.txt", "a") as p:
        p.write("%s, %s, %s, %s\n" % (oper, sbproc, proc, counter))
        print("总大小：%s" %(oper + sbproc + proc + counter))
